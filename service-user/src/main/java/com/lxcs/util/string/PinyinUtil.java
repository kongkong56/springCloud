...
public class PinyinUtil {
	 ...
	 * @return 汉语拼音首字母
	 */
	public static String cn2singelSpell(String chinese) {
		...
		return pybf.toString().replaceAll("\\W", "").replaceAll("99", " ");
	}
	/**
	 * 获取汉字串拼音首字母，英文字符不变
	 *
	...
	public static String cn2Spell(String chinese) {
		...
		for (int i = 0; i < arr.length; i++) {
			if (arr[i] > 128) {
				try {
					String[] hanyupinyinArrays = PinyinHelper
							.toHanyuPinyinStringArray(arr[i], defaultFormat);
					if (hanyupinyinArrays.length > 0) {
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		System.out.println(cn2singelSpell(x));
	}
}
